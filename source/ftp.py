...
def connect_ftp():
    ftp=FTP()
    ftp.connect(host=input(Fore.RESET + "ingresa el host/pagina: "), port=int(input("ingresa el puerto FTP: ")), timeout=-999)
...
delete(filename) - Remove the file named filename from the server. If successful, returns the text of the response, otherwise raises error_perm on permission errors or error_reply on other errors.
